 ...
 */
@Deprecated
public class FeatureDescriptor2<T extends RealType<T>> implements Externalizable {
	 ...
	 */
	public FeatureDescriptor2(final boolean numInputChannels) {
	 ...
	 */
	public FeatureDescriptor2(final boolean numInputChannels, final boolean convertToLab, final boolean firstDerivative,
	...
	public RandomAccessibleInterval<FloatType> apply(RandomAccessibleInterval<T> in) {
		...
		if (isAddFirstDerivative()) {
			...
			final List<RandomAccessibleInterval<FloatType>> derivatives = computeDerivatives(inFloatType, m_ops);
			list.addAll(derivatives);
		}

		if (isAddHoG()) {
			// compute hog features and add them to the list
			final RandomAccessibleInterval<FloatType> hog = (RandomAccessibleInterval<FloatType>) m_ops
		...
		if (isApplyMinMax()) {
			...
			while (!list.isEmpty()) {
				// min
				final RandomAccessibleInterval<FloatType> min = m_ops.create()
						.img(new FinalInterval(in.dimension(0), in.dimension(1)), new FloatType());
				m_ops.filter().min(Views.flatIterable(min), list.get(0), shape);
				listMaxMin.add(min);
				// max
				final RandomAccessibleInterval<FloatType> max = m_ops.create()
						.img(new FinalInterval(in.dimension(0), in.dimension(1)), new FloatType());
				m_ops.filter().max(Views.flatIterable(max), list.remove(0), shape);
				listMaxMin.add(max);
			}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof FeatureDescriptor2)) {
			return false;
		}
		FeatureDescriptor2<?> other = (FeatureDescriptor2<?>) obj;
